...
Требуется найти распределение компонентов по фазам, при известных молярных долях компонентов $c_i, \; i = 0 \dividesymbol N_{LO} + N_{HO} + N_{G} + N_{S}$, давлении $p$ и температуре $T$, неявно определенной молярной энтальпией системы $\eta$. Также требуется вычислить температуру смеси.
...
\item $h_i(p,T)$ --- молярные энтальпии компонентов в ``основной'' фазе. Для воды и легких нефтей ``основной'' фазой является $G$.
\end{itemize}
